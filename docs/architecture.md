...
Отдельного UI-режима нет. Детектор опирается на лодыжки/стопы; пороги MediaPipe снижены, чтобы поза находилась и при кадре от пояса вниз. Overlay рисует только видимые точки.
